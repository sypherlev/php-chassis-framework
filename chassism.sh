...
if [ $1 = "-m" ]; then
    ...
    output=$($chphpexe web/index.php "Chassis\\Migrate\\Migrate:migrateUnapplied" $2)
    echo "$output"
    exit 0
fi
if [ $1 = "-c" ]; then
    ...
    output=$($chphpexe web/index.php "Chassis\\Migrate\\Migrate:createMigration" $2 $3)
    echo "$output"
    exit 0
fi
if [ $1 = "-b" ]; then
    ...
    output=$($chphpexe web/index.php "Chassis\\Migrate\\Migrate:bootstrap" $2 $3)
    echo "$output"
    exit 0
fi