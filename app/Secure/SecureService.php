...
abstract class SecureService
{
    ...
    protected function setRolesByAuthHeader(SourceInterface $source) {
        $usersource = new AuthData($source);
        $authkey = $this->getAuthHeader();
        $cookietoken = $this->getCookieToken();
        if($authkey) {
            $this->currentUser = $usersource->findUserByAuth($authkey, $cookietoken);
            $roles = $usersource->findRolesByAuth($authkey, $cookietoken);
            if(is_array($roles)) {
    ...
    protected function setRolesByPost(SourceInterface $source) {
        $usersource = new AuthData($source);
